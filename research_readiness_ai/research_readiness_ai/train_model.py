...
IMPORTANT: There is no public 10,000-paper labeled dataset of "draft vs.
published" papers with these exact 24 metrics. To make the full pipeline
runnable end-to-end today, this script generates a synthetic-but-realistic
training set: it samples plausible metric values and derives a readiness
label from a weighted formula plus noise, mirroring how reviewers actually
weight these signals (methodology and paper structure matter most, dataset
quality next, literature review last).

When you have real annotated papers (see "Next Steps: Collect Labeled
Dataset" in your deck), replace `generate_synthetic_dataset()` with a
loader that reads your real feature/label CSV and re-run this script.
The rest of the pipeline (feature order, SHAP, recommendations) does not
need to change.
"""
